...
const getMostRecentDB = "SELECT MAX(rowIdNum), userid, activity, date, amount FROM ActivityTable";
 ...
 */
async function post_activity(userid, activity) {
  try {
    await db.run(insertDB, act.ActivityToList(userid, activity));
  } catch (error) {
