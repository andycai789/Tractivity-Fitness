...
app.get('/name', isAuthenticated, async function(request, response, next) {
  console.log("Server recieved a get /name request at", request.url);
  const searchPDB = "select * from ProfileTable where userId = ?";
  let name = await db.get(searchPDB, [request.user.userData]);
  response.send(name);
});
...
app.post('/store', isAuthenticated, async function(request, response, next) {
  ...
  await dbo.post_activity(request.user.userData, activity)
...
app.get('/reminder', isAuthenticated, async function(request, response, next) {
  ...
  let result = await dbo.get_most_recent_planned_activity_in_range(request.user.userData, 0, currTime)
  await dbo.delete_past_activities_in_range(request.user.userData, 0, currTime);
...
app.get('/week', isAuthenticated, async function(request, response, next) {
  ...
  let max = date
  let result = await dbo.get_similar_activities_in_range(request.user.userData, activity, min, max)
...
async function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    const searchPDB = "select * from ProfileTable where userId = ?";
    const insertPDB = "insert into ProfileTable (userId, firstName) values (?,?)";

    try {
      let result = await db.get(searchPDB, [userid]);
      if (result === undefined) {
        await db.run(insertPDB, [userid, firstname]);
      }
    }
    catch (error) {
      console.log("gotProfile Error: " + error);
    }
    console.log("All Profiles");
    console.log(await db.all('SELECT * FROM ProfileTable'));
